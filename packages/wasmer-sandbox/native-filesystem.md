...
Protocol version 3 uses a u32 little-endian outer frame length, a version byte,
...
Request metadata contains scope, token, command ID, sequence and an operation;
responses contain
`value` or an errno `code`, followed by read bytes when applicable.

Each command installs an authoritative record in the resident owning cell. The
record contains the cell scope, command ID, owning activation epoch, random
execution token, absolute deadline, next sequence, one connection binding and
an explicit guest operation mask. The mask admits `stat`, `list`, `mkdir`,
`open`, `close`, `fstat`, `read`, `write`, `truncate`, `unlink`, `rmdir`, `rename`,
`sync` and `heartbeat`; application-only operations are excluded. The grant is
installed before the supervisor starts the helper. It is process-local, never
persisted, and cannot outlive an owner activation.

The first request proves possession of the token and matches the record's
scope, command ID and epoch. The server binds that grant to a private connection
session ID generated by celld. Each later frame must match the bound scope,
token and command ID; these wire fields cannot select another grant. The server
uses the bound activation epoch for owner checks and the cell validates the
connection session, operation mask, deadline and exact sequence before the
input gate, transaction or file contents are inspected. Authentication failures
close the socket. Ordinary filesystem errors consume the sequence. A closed
input gate returns `EBUSY` without consuming it, but the production helper
terminates the command on this condition. At most 32 socket connections are
active, and a grant permits at most 100,000 requests. Partial frames and active
requests have five-second deadlines; idle connections have 310 seconds.
The supervisor separately enforces command deadlines.

Completion and cancellation revoke the grant immediately. Disconnect revokes
the exact bound grant and its guest handles. A lost connection cannot reconnect
with the old token: the caller must start a new command with a new grant. A
second simultaneous connection cannot take over the active grant. Expired
grants and changed owner epochs fail every subsequent operation, including
metadata, sync and heartbeat; owner retirement discards process-local grants
and handles. Cleanup of an old session checks the epoch and session ID so it
cannot revoke a replacement execution. An idle socket may remain allocated
until its timeout, but it carries no usable authority after the deadline.
...
Version 2 clients are rejected. The helper and celld must both use version 3;
there is no mixed-version grace period or automatic fallback. The HTTP
implementation is retained only as an explicit reference/remote backend
(`nativeFilesystem: false` / `SANDBOX_NATIVE_FILESYSTEM=0`). Its configuration and
filesystem callback token are separate. It retains its own in-activation
token/sequence checks and does not use native socket grants or connection
binding. Native helpers receive neither a callback URL nor a callback token.
Execute/cancel and command results still use the supervisor's authenticated
HTTP control API. The socket directory admits same-UID processes, so this grant
limits access by a helper or peer that lacks another command's token; it is not
an OS boundary against a compromised same-UID native process. That boundary
requires the later microVM/OS isolation stage.
