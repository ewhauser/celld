...
impl File {
    pub fn write(&mut self, buf: &[u8], cursor: &mut u64) -> io::Result<usize> {
        ...
        // Reserve the entire write before changing an existing prefix or
        // materializing a sparse gap. Rejected growth is an atomic failure.
        self.buffer.reserve_exact(end.saturating_sub(self.buffer.len()))?;
