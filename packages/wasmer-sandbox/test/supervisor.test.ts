...
for (const native of [false, true])
  test(`supervisor ${native ? "IPC without HTTP credentials" : "HTTP"}: authenticates, pins modules, bounds admission, cancels and deduplicates`, async () => {
    const dir = await mkdtemp(tmpdir() + "/celld-supervisor-");
    await writeFile(dir + "/tool", "synthetic module");
    await writeFile(
      dir + "/runner",
      `#!${process.execPath}\nlet text='';process.stdin.on('data',b=>text+=b);process.stdin.on('end',()=>{const c=JSON.parse(text);if(c.nativeFilesystem && (c.callback || c.callbackToken))throw Error("IPC helper received HTTP credentials");setTimeout(()=>console.log(JSON.stringify({reason:'exited',exitCode:0,stdout:c.args.join(' '),stderr:''})),c.args[0]==='wait'?10000:10);});\n`,
      { mode: 0o700 },
    );
    ...
    const config = {
      ...
      token,
      ...(native
        ? { filesystemSocket: "/tmp/test-agentfs.sock" }
        : { callbackToken: token, callbackOrigin: "http://127.0.0.1:19876" }),
      runner: dir + "/runner",
      maxConcurrent: 1,
      tools: {
        guest: {
    ...
    const run = {
      ...
      token: execToken,
      args: ["hello"],
      ...(native ? { nativeFilesystemScope: "Workspace:w" } : {}),
    };
    ...
    try {
      ...
      if (native)
        assert.equal(
          (await post("/v1/run", { ...run, nativeFilesystemScope: undefined }))
            .status,
          400,
        );
      assert.equal(
      ...
      for (let i = 0; i < 50; i++) {
        if (((await (await fetch(url + "/healthz")).json()) as any).active)
          break;
